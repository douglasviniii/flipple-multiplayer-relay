...
async fn gate_a_routes_raw_ip_datagrams_in_both_directions() {
    ...
    let (guest_endpoint, guest) = connect_peer(relay_address, certificate_der, 2).await;

    let host_packet = Bytes::from_static(&[
        0x45, 0x00, 0x00, 0x1c, 0, 0, 0, 0, 0x40, 0x11, 0, 0, 100, 96, 0, 1, 100, 96, 0, 2, 0x4a,
        0x7e, 0x4a, 0x7e, 0, 8, 0, 0,
    ]);
    ...
    let guest_packet = Bytes::from_static(&[
        0x45, 0x00, 0x00, 0x1c, 0, 1, 0, 0, 0x40, 0x11, 0, 0, 100, 96, 0, 2, 100, 96, 0, 1, 0x4a,
        0x7e, 0x4a, 0x7e, 0, 8, 0, 0,
    ]);
...
async fn connect_peer(
    ...
    assert_eq!(response.room_id, "gate-a-room");
    (endpoint, connection)
}
